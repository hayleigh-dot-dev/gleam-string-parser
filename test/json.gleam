...
pub fn json_test () {
    ...
    let ast = JsonObject([
        tuple("foo", JsonNumber(3.14)),
        tuple("bar", JsonArray([ JsonString("hello"), JsonNull ]))
    ])
...
fn json_object_parser () -> Parser(JSON) {
    let pair = fn (a, b) { tuple(a, b) }

    let separator = parser.spaces() 
        |> parser.drop(parser.string(",")) 
        |> parser.drop(parser.spaces())

    let key_parser = parser.succeed(function.identity)
        |> parser.drop(parser.string("\""))
        |> parser.keep(parser.take_while(fn (c) { c != "\"" }))
        |> parser.drop(parser.string("\""))

    let key_value_pair = parser.succeed2(pair)
